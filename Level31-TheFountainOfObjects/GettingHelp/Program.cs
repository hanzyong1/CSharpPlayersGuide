...
switch (gameSize)
{
    case "small":
        ...
        break;
    case "medium":
        world.InitialiseWorld(6, 6);
        break;
    case "large":
        world.InitialiseWorld(8, 8);
        break;
}
...
public class World
{
    ...
    public void InitialiseWorld(int numRows, int numCols)
    {
        ...
        for (int row = 0; row < Rooms.GetLength(0); row++)
        {
            for (int column = 0; column < Rooms.GetLength(1); column++)
            {
                if (row == 0 && column == 0)
                {
                    Rooms[row, column] = new Room(true, false, false);
                }
                else if (row == 0 && column == 2)
                {
                    Rooms[row, column] = new Room(false, true, false);
                }
                else if (row == 1 && column == 1)
                {
                    Rooms[row, column] = new Room(false, false, true);
                }
                else
                {
    ...
    {
        Console.WriteLine("move north - move your character north");
        Console.WriteLine("move south - move your character south");
        Console.WriteLine("move east - move your character east");
        Console.WriteLine("move west - move your character west");
        Console.WriteLine("enable fountain - activate fountain");
        Console.WriteLine("disable fountain - deactivate fountain");
    }
}
...
public class Room
{
    ...
    public bool HasPit { get; } = false;

    public Room()
    {
    }

    public Room(bool isEntrance, bool hasFountain, bool hasPit)
    {
